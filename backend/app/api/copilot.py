...
async def chat(
    ...
    if "orphan" in text:
        orphaned = await db.find_many("agents", {"status": "orphaned"}, sort=[("risk", -1)])
        if not orphaned:
    ...
    if "pii" in text or "customer" in text or "sensitive" in text:
        risky = await db.find_many("agents", {"risk": {"$gte": 75}}, sort=[("risk", -1)])
        names = ", ".join(a.name for a in risky[:5]) or "none"
        return ChatOut(reply=f"Agents likely touching sensitive data (risk >= 75): {names}.")

    if "alert" in text:
        open_alerts = await db.find_many("alerts", {"resolved": False}, sort=[("created_at", -1)])
        if not open_alerts:
